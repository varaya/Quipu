#  NtsD.pm - Registra y contabiliza Notas de Débito emitidas o recibidas 
...
sub crea {
	...
	# Define campos para datos generales de la ND
	$documento = $mDatosC->LabEntry(-label => "ND # ", -width => 12,
	...
	$nombre->grid(-row => 1, -column => 1, -columnspan => 2, -sticky => 'ne');
	$fecha->grid(-row => 2, -column => 0, -columnspan => 2, -sticky => 'nw');
...
sub validaFecha ( $ $ $ $ ) 
{
	...
	if ( not $$v =~ m|\d+/\d+/\d+| ) {
		...
		$$c->focus;
	} elsif ( not $ut->analizaFecha($$v) ) { # Valida la fecha
		print chr 7 ;
...
{ 
	...
	if ($Documento eq '') {
		$Mnsj = "Primero registre número de la ND";
		$documento->focus;
		return;
	}
...
sub inicializaV ( )
{
	...
	$Codigo = $RUT = $Glosa = $Detalle = $CtaIVA = $CtaT = $NCCto = $CCto = '';
